          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-surface-container-lowest border border-surface-container-highest rounded-xl shadow-lg p-space-md z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="flex items-center justify-between pb-space-xs border-b border-surface-container-highest">
                ...
                <span className="font-label-sm text-label-sm text-primary font-medium">{unreadCount} Active</span>
              </div>
              <div className="flex flex-col gap-space-sm mt-space-sm max-h-72 overflow-y-auto">
                {notifications.length === 0 && (
                  <div className="p-space-sm text-[12px] text-on-surface-variant text-center">
                    No notifications yet.
                  </div>
                )}
                {notifications.slice(0, 6).map((notification) => {
                  const isAlert = notification.type === 'battery_health_change' || notification.type === 'data_quality_warning';
                  return (
                    <div
                      key={notification.id}
                      onClick={() => {
                        if (!notification.read) markNotificationRead(notification.id);
                        if (notification.vehicleId) selectVehicleForReport(notification.vehicleId);
                        setShowNotifications(false);
                      }}
                      className={`p-space-sm rounded-lg flex items-start gap-space-sm cursor-pointer transition-colors ${
                        notification.read
                          ? 'bg-surface-container-low hover:bg-surface-container'
                          : isAlert
                          ? 'bg-error-container/20 border border-error-container/40 hover:bg-error-container/30'
                          : 'bg-primary-container/10 border border-primary-container/30 hover:bg-primary-container/20'
                      }`}
                    >
                      <span className={`material-symbols-outlined text-[18px] mt-0.5 ${isAlert ? 'text-error' : 'text-primary'}`}>
                        {isAlert ? 'warning' : 'check_circle'}
                      </span>
                      <div className="flex flex-col">
                        <span className={`font-label-sm text-label-sm font-semibold ${isAlert && !notification.read ? 'text-error' : 'text-on-surface'}`}>
                          {notification.title}
                        </span>
                        <span className="text-[11px] text-on-surface-variant">{notification.message}</span>
                        <span className="text-[10px] text-on-surface-variant mt-1">{notification.relativeTime}</span>
                      </div>
                    </div>
                  );
                })}
