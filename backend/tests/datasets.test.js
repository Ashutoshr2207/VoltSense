...
describe('Datasets API', () => {
  ...
  const validCsv = 'Cycle,Capacity,Voltage_measured,Current_measured,Temperature_measured,Time\n1,1.85,3.95,-1.5,24.5,0.0\n1,1.85,3.90,1.5,25.0,600.0';
  ...
  describe('POST /api/vehicles/:vehicleId/datasets', () => {
    it('should upload CSV telemetry file and return 201 with dataset record', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${tokenA}`)
        .attach('file', Buffer.from(validCsv), 'bms_test.csv')
    ...
    it('should return 403 FORBIDDEN if another user tries to upload to vehicle', async () => {
      const res = await request(app)
        ...
        .attach('file', Buffer.from(validCsv), 'telemetry.csv');
